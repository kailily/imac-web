...
function AcademyMap() {
  ...
  const dormPositions = [
    { id: "yuehua", name: "月华阁", en: "Moonlight Pavilion", angle: 162, status: "explored" },
    { id: "tingxue", name: "听雪楼", en: "Snow Listener Tower", angle: 342, status: "explored" },
    { id: "qingteng", name: "青藤苑", en: "Ivy Court", angle: 270, status: "partial" },
    { id: "wangshan", name: "望山居", en: "Mountain View Residence", angle: 198, status: "unexplored" },
    { id: "guanxing", name: "观星台", en: "Stargazer Platform", angle: 90, status: "partial" },
  ];
  ...
  const dormDetails = {
    yuehua: {
      name: "月华阁", en: "Moonlight Pavilion", type: "学生宿舍区", structure: "三间独立套房",
      adjacent: "植物园（温室）、实验楼",
      status: "explored",
    ...
    tingxue: {
      name: "听雪楼", en: "Snow Listener Tower", type: "学生宿舍区", structure: "三间独立套房",
      adjacent: "音乐厅、美术馆",
      status: "explored",
    ...
    qingteng: {
      name: "青藤苑", en: "Ivy Court", type: "学生宿舍区", structure: "三间独立套房",
      adjacent: "主教学楼、图书馆",
      status: "partial",
    ...
    guanxing: {
      name: "观星台", en: "Stargazer Platform", type: "学生宿舍区", structure: "三间独立套房",
      adjacent: "体育馆",
      status: "partial",
    ...
    wangshan: {
      name: "望山居", en: "Mountain View Residence", type: "学生宿舍区", structure: "三间独立套房（推测）",
      adjacent: "外围围墙、实验楼",
      status: "unexplored",
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case "explored": return "#4a7c59";
      case "partial": return "#c49a2c";
      case "unexplored": return "#c42828";
      case "core": return "#7a3ab0";
      default: return "#888";
    }
  };
  ...
  const getStatusLabel = (status) => {
    switch (status) {
      ...
      case "partial": return "部分探索";
      case "unexplored": return "未探索";
      case "core": return "核心区域";
      default: return "未知";
    }
  };
  ...
  const pentagramPath = pentLineOrder.map((idx, i) =>
    `${i === 0 ? "M" : "L"} ${pentPoints[idx].x} ${pentPoints[idx].y}`
  ).join(" ");
  ...
  const selected = selectedDorm ? { ...selectedDorm, isDorm: true } : selectedPublic ? { ...selectedPublic, isDorm: false } : null;
  ...
  return (
    <>
      ...
      <div className="academy-map-container">
        ...
        <svg className="academy-map-svg" viewBox="0 0 720 560" xmlns="http://www.w3.org/2000/svg">
          {/* Outer wall - circular with irregular perimeter */}
          <circle cx={CX} cy={CY} r={R + 60} className="outer-wall" />

          {/* Pentagram (star) lines */}
          <path d={pentagramPath} className="pentagram-line" />
          ...
          {publicBuildings.map((b) => {
            ...
            if (b.id === "garden") {
              ...
              return (
                <g key={b.id} onClick={() => handleClick(b.id)} style={{ cursor: "pointer" }}>
                  <circle
                  ...
                  />
                  <circle cx={pos.x} cy={pos.y} r={30} fill="rgba(122, 58, 176, 0.1)" stroke="#7a3ab0" strokeWidth="1" />
                  <circle cx={pos.x} cy={pos.y} r={18} fill="rgba(122, 58, 176, 0.08)" stroke="#7a3ab0" strokeWidth="0.5" />
                  {/* Rose icon */}
                  <g className="rose-icon" transform={`translate(${pos.x - 8}, ${pos.y - 10})`}>
                    <path d="M8 0 C12 4 14 8 10 14 C6 10 4 6 8 0Z" fill="#7a3ab0" opacity="0.7"/>
                    <path d="M8 0 C4 4 2 8 6 14 C10 10 12 6 8 0Z" fill="#b88ed9" opacity="0.5"/>
                  </g>
                  <text x={pos.x} y={pos.y + 56} className="dorm-label" style={{ fill: "#b88ed9", fontSize: "11px" }}>{b.name}</text>
                </g>
              );
            }
            ...
            return (
              <g key={b.id} onClick={() => handleClick(b.id)} style={{ cursor: "pointer" }}>
                ...
                />
                <text x={pos.x} y={pos.y + 4} className="public-label">{b.name}</text>
              </g>
            );
          })}

          {/* Dorm buildings - pentagons at star points */}
          {dormPositions.map((d) => {
            ...
            return (
              <g key={d.id} onClick={() => handleClick(d.id)} style={{ cursor: "pointer" }}>
                <polygon
                ...
                {/* Name label outside */}
                <text x={labelX} y={labelY} className="dorm-label" style={{ fontSize: "13px" }}>{d.name}</text>
                <text x={labelX} y={labelY + 13} className="dorm-label-en">{d.en.toUpperCase()}</text>
              </g>
            );
          })}
        </svg>
        ...
        <div className="map-legend">
          <div className="map-legend-item">
            <div className="map-legend-dot" style={{ borderColor: "#4a7c59", background: "rgba(74, 124, 89, 0.2)" }}></div>
            <span>已探索</span>
          </div>
          <div className="map-legend-item">
            <div className="map-legend-dot" style={{ borderColor: "#c49a2c", background: "rgba(196, 154, 44, 0.2)" }}></div>
            <span>部分探索</span>
          </div>
          <div className="map-legend-item">
            <div className="map-legend-dot" style={{ borderColor: "#c42828", background: "rgba(196, 40, 40, 0.2)" }}></div>
            <span>未探索</span>
          </div>
          <div className="map-legend-item">
            <div className="map-legend-dot" style={{ borderColor: "#7a3ab0", background: "rgba(122, 58, 176, 0.2)", borderRadius: "50%" }}></div>
            <span>核心区域</span>
          </div>
          <div className="map-legend-item">
            <div className="map-legend-dot" style={{ borderColor: "var(--text-tertiary)", clipPath: "polygon(50% 0%, 100% 38%, 82% 100%, 18% 100%, 0% 38%)" }}></div>
            <span>宿舍区</span>
          </div>
        </div>
      </div>
      ...
      {selected && (
        <div className="building-detail-panel">
          <div className="building-detail-head">
            <div className="building-detail-title-group">
              <h3>{selected.name}</h3>
              <div className="en">{selected.en.toUpperCase()}</div>
            </div>
            <div className="building-detail-meta">
              <span
                className="building-detail-badge"
                style={{ borderColor: getStatusColor(selected.status), color: getStatusColor(selected.status) }}
              >
                {getStatusLabel(selected.status)}
              </span>
              <span className="building-detail-badge" style={{ borderColor: "var(--text-muted)", color: "var(--text-secondary)" }}>
                {selected.isDorm ? selected.type : `${selected.floors} · 公共建筑`}
              </span>
          ...
          <div className="building-detail-body">
            ...
            {selected.isDorm && (
              <div className="building-detail-section">
                <h4>结构与相邻</h4>
                <p>
                  <strong>结构：</strong>{selected.structure}<br/>
                  <strong>相邻建筑：</strong>{selected.adjacent}
                </p>
              </div>
            )}
            <div className="building-detail-section">
            ...
            <div className="building-detail-section full">
              <h4>探索记录</h4>
              <p>{selected.record}</p>
            </div>
          </div>
