...
function ProfileCenterPage() {
  ...
  const savedProfile = (() => {
    try { return JSON.parse(localStorage.getItem("imac_profile_data") || "null"); } catch (e) { return null; }
  })();
                  ...
                  <div className="pc-card-body">
                    <div className="pc-profile-head">
                      ...
                      <button className="pc-edit-btn" onClick={() => {
                        if (editing) {
                          try { localStorage.setItem("imac_profile_data", JSON.stringify(profile)); } catch (e) {}
                        }
                        setEditing(!editing);
                      }}>
                        {editing ? "保存" : "编辑资料"}
                    ...
                    <div className="pc-info-grid">
                      {[
                        ...
                        { key: "org", label: "所属组织" },
                        { key: "id", label: "IMAC编号" },
                        { key: "location", label: "所在地区" },
                        { key: "age", label: "年龄" },
                        { key: "specialty", label: "擅长领域" },
                        { key: "experience", label: "个人经历" },
                        ...
                        <div key={f.key} className="pc-info-row">
                          ...
                          <span className="pc-info-value">
                            {editing && ["code", "name", "contact", "anchor", "location", "age", "specialty", "experience"].includes(f.key) ? (
                              <textarea
                                rows={f.key === "experience" || f.key === "specialty" ? 2 : 1}
                                value={profile[f.key]}
                                onChange={(e) => setProfile({ ...profile, [f.key]: e.target.value })}
                                style={{ width: "100%", background: "rgba(10,10,14,0.8)", border: "1px solid var(--border-light)", color: "var(--text-primary)", fontFamily: "inherit", fontSize: "13px", padding: "6px 8px", borderRadius: "4px", resize: "vertical" }}
                              />
                            ) : profile[f.key]}
                          </span>
                        </div>
