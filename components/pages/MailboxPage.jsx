function MailboxPage() {
  ...
  const [mails, setMails] = React.useState(() => {
    ...
    return mailList.map((m) => (readIds.includes(m.id) ? { ...m, unread: false } : m));
  });
  ...
  const syncUnread = React.useCallback((list) => {
    ...
    // 触发当前页其他组件监听
    window.dispatchEvent(new CustomEvent("mail-unread-changed", { detail: list.filter((m) => m.unread).length }));
  }, []);
  ...
  const markAllRead = () => {
    const next = mails.map((m) => ({ ...m, unread: false }));
    setMails(next);
  };
    ...
    <div className="mailbox-page">
      ...
      <section className="mail-hero">
        <div className="container">
          <div className="mail-hero-inner">
            <div className="mail-title-group">
              <span className="mail-label">IMAC · INTERNAL MAIL SYSTEM</span>
              <h1 className="mail-title">系统邮箱</h1>
            </div>
            <div className="mail-back" onClick={() => navigate("/portal")}>
        ...
        <div className="container">
          <div className="mail-layout">
            ...
            <div className="mail-sidebar">
              <div className="mail-toolbar">
                <div className="mail-toolbar-left">
                  <button className={`mail-btn ${filter === "all" ? "active" : ""}`} onClick={() => setFilter("all")}>全部</button>
                  <button className={`mail-btn ${filter === "unread" ? "active" : ""}`} onClick={() => setFilter("unread")}>未读</button>
                </div>
                <button className="mail-compose-btn" onClick={() => setComposing(!composing)}>
                  {composing ? "取消" : "写邮件"}
                </button>
              </div>
              ...
              {composing && (
                <div className="mail-composer">
                  <div className="form-field">
                    <div className="form-label">收件人</div>
                    <input className="form-input" placeholder="收件人代号或部门" />
                  </div>
                  <div className="form-field">
                    <div className="form-label">主题</div>
                    <input className="form-input" placeholder="邮件主题" />
                  </div>
                  <div className="form-field">
                    <div className="form-label">正文</div>
                    <textarea className="form-textarea" placeholder="邮件内容..."></textarea>
                  </div>
                  <button className="mail-compose-btn" onClick={() => { alert("邮件已发送（模拟）"); setComposing(false); }}>
                    发送
                  </button>
                </div>
              )}

              <div className="mail-unread-num">未读 {unreadCount} 封 · 共 {mails.length} 封</div>
              <div className="mail-toolbar" style={{ paddingTop: 0, borderBottom: "1px solid var(--border-color)" }}>
                <button className="mail-btn" onClick={markAllRead}>全部标为已读</button>
              </div>
                  ...
                  >
                    <div className="mail-item-head">
                      <span className="mail-item-from">
                        {m.unread && <span className="mail-unread-dot"></span>}
                        {m.from}
                      </span>
                      <span className="mail-item-time">{m.time}</span>
                    </div>
                    <div className="mail-item-subject">
                      {m.important && <span className="mail-item-imp">★ </span>}
                      {m.subject}
                    </div>
                    <div className="mail-item-summary">{m.summary}</div>
                  </div>
                ))}
                {filteredMails.length === 0 && (
                  <div style={{ padding: "30px", textAlign: "center", color: "var(--text-tertiary)", fontSize: "12px" }}>
                    暂无邮件
                  </div>
                )}
              </div>
            ...
            <div className="mail-content">
              {selectedMail ? (
                <>
                  <div className="mail-content-subject">
                    {selectedMail.important && <span style={{ color: "var(--accent-red-bright)", marginRight: "8px" }}>★</span>}
                    {selectedMail.subject}
                  </div>
                  <div className="mail-content-meta">
                    <span className="mail-content-from">
                      <strong>发件人：</strong>{selectedMail.from}
                    </span>
                    <span className="mail-content-date">安珀历39年 · {selectedMail.time}</span>
                  </div>
